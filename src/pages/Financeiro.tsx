...
const Financeiro = () => {
  ...
  const handleExportRateioPDF = () => {
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head><title>Rateio</title><style>body{font-family:Arial,sans-serif;margin:40px;color:#333}h1{font-size:18px;margin-bottom:4px}p{font-size:12px;color:#666;margin:2px 0}table{width:100%;border-collapse:collapse;margin-top:20px;font-size:11px}th,td{padding:6px 8px;border:1px solid #ddd;text-align:left}th{background:#f5f5f5;font-weight:600}tfoot td{background:#f5f5f5;font-weight:bold}.footer{margin-top:30px;font-size:10px;color:#999;border-top:1px solid #ddd;padding-top:10px}</style></head><body><h1>Vinci Compass — Relatório de Rateio de Consumo</h1><p>Período: Março/2026 | Ativo: 360JK</p><p>Método: ${method}</p><table><thead><tr><th>Locatário</th><th style="text-align:center">Andar(es)</th><th style="text-align:right">Área (m²)</th><th style="text-align:right">%</th><th style="text-align:right">Condomínio</th><th style="text-align:right">Utilities</th><th style="text-align:right">Total</th></tr></thead><tbody>${tableRows}</tbody><tfoot><tr><td colspan="2"><strong>TOTAL</strong></td><td style="text-align:right">${totalArea.toLocaleString('pt-BR')}</td><td style="text-align:right">100%</td><td style="text-align:right">R$ ${totalCondominial.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td><td style="text-align:right">R$ ${totalUtilities.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td><td style="text-align:right">R$ ${totalToRateio.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td></tr></tfoot></table><div class="footer">Gerado em ${new Date().toLocaleString('pt-BR')} | Vinci Compass | 360JK</div></body></html>`);
